...
__export(index_exports, {
  ...
  VERSION: () => VERSION,
  applyFiltersOnRecord: () => applyFiltersOnRecord,
  applyFiltersOnReplay: () => applyFiltersOnReplay,
  createBobina: () => createBobina,
  createEngine: () => createEngine,
  emptyCassette: () => emptyCassette,
  findInteraction: () => findInteraction,
  loadCassette: () => loadCassette,
  matchRequest: () => matchRequest,
  resetBobinaSingleton: () => resetBobinaSingleton,
  saveCassette: () => saveCassette,
...
var import_node = __toESM(require("@mswjs/interceptors/presets/node"), 1);
...
function cassettePath(dir, name) {
  ...
  return (0, import_node_path.join)(dir, `${name}.json`);
}
...
async function saveCassette(dir, cassette) {
  ...
  await (0, import_promises.writeFile)(file, `${JSON.stringify(cassette, null, 2)}
`, "utf8");
}
...
async function toRecordedResponse(response) {
  const bytes = import_node_buffer.Buffer.from(await response.clone().arrayBuffer());
  return {
...
function buildResponse(recorded) {
  const body = NULL_BODY_STATUSES.has(recorded.status) ? null : recorded.bodyEncoding === "base64" ? import_node_buffer.Buffer.from(recorded.body, "base64") : recorded.body;
...
function createEngine(opts) {
  ...
  const interceptor = new import_interceptors.BatchInterceptor({ name: "bobina", interceptors: import_node.default });
  let current = null;
  let dirty = false;
